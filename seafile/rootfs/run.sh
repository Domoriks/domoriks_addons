...
# Increase nginx buffer sizes to handle browser headers (fixes 400 "Request Header Too Large" errors)
echo "[INFO] Increasing nginx buffer size limits..."
find /etc/nginx -type f -name '*.conf' -exec sed -i '1i client_max_body_size 10G;\nlarge_client_header_buffers 4 32k;\nclient_body_buffer_size 256k;' {} \; 2>/dev/null || true
